...
class SFTP extends EventEmitter {
    ...
    write(handle, buf, off, len, position, cb) {
        //console.log("--write--")
        //console.log(off + ":" + len + ":" + buf.length)

        //console.log("Write FINAL: " + buf);
        if (this.server)
            throw new Error('Client-only method called in server mode');
        ...
        if (off > buf.length)
            throw new Error('offset is out of bounds');
        if (off + len > buf.length)
            throw new Error('W length extends beyond buffer');
        if (position === null)
            throw new Error('null position currently unsupported');
    ...
    fastPut(localPath, remotePath, opts, cb) {
        ...
        fastXfer1(fs, this, localPath, remotePath, opts, cb);
    }
...
function fastXfer(src, dst, srcPath, dstPath, opts, cb) { //original
  console.log('fastXfer--');
    let concurrency = 64;
    let chunkSize = 32768   
    let onstep;
    let mode;
    ...
    /*
      chunksize = 10, concurrency = 2, fsize = 162 > nb = 10, bufsize: 20
      chunksize = 10, concurrency = 1, fsize = 162 > nb = 10, bufsize: 10
      chunksize = 30, concurrency = 1, fsize = 162 > nb = 30, bufsize: 30
      chunksize = 30, concurrency = 2, fsize = 162 > nb = 30, bufsize: 60
      chunksize = 50, concurrency = 10, fsize = 162 > nb = 50, bufsize: 150
      chunksize = 32768, concurrency = 10, fsize = 162 > nb = 162, bufsize: 162
      chunksize = 161, concurrency = 10, fsize = 162 > nb = 161, bufsize: 161

      chunksize = 32768, concurrency = 64, fsize = 117005824 > nb = 32768, bufsize: 2097152
      chunksize = 32768, concurrency = 64, fsize = 162 > nb = 162, bufsize: 162
        ...
        function tryStat(err, attrs) {
            ...
            dst.open(dstPath, 'w', (err, destHandle) => {
                ...
                function onread(err, nb, data, dstpos, datapos, origChunkLen) {
                    ...
                    //let readbuf2 = Buffer.from(word, "utf-8")
                    //console.log("write: " +  data)
                    //console.log("write 2: " +  readbuf)
                    //readbuf = Buffer.from('1234567890');


                    dst.write(dstHandle, readbuf, datapos, nb, dstpos, writeCb); //here1
                    ...
                    function writeCb(err) {
                        ...
                        total += nb;
                        console.log('writeCb Total: ' + total);
                        onstep && onstep(total, nb, fsize);

                        if (nb < origChunkLen)
                            return singleRead(datapos, dstpos + nb, origChunkLen - nb);

                        console.log("total: " + total + ", fsize:" + fsize);
                        if (total === fsize) {
                ...
                function makeCb(psrc, pdst, chunk) {
                    return (err, nb, data) => {
                        console.log("nb: " + nb + ",pdst: " + pdst + ",psrc: " + psrc + ",chunk:" + chunk)
                        console.log("readbuf.length: " + readbuf.length)
                        console.log(readbuf.toString())
                        
                        onread(err, nb, readbuf, pdst, psrc, chunk);
                    };
                }

                function singleRead(psrc, pdst, chunk) {
                    console.log("--singleRead--:" + psrc + ", pdst:" + pdst)
                    src.read(srcHandle,
                        readbuf,
                        psrc,
                        chunk,
                ...
                function startReads() {
                    console.log("--startReads--");
                    let reads = 0;
                    let psrc = 0;
                    console.log(pdst + ":" + fsize + ":" + reads + ":" + concurrency);
                    while (pdst < fsize && reads < concurrency) {
                        const chunk =
                            (pdst + chunkSize > fsize ? fsize - pdst : chunkSize);
                        console.log("psrc: " + psrc + ", pdst:" + pdst + ", chunk:" + chunk)
...
function fastXfer1(src, dst, srcPath, dstPath, opts, cb) {
  console.log('fastXfer1--');
    let concurrency = 64; //1; //64;
    let chunkSize = 152768 //10; //32768
    let onstep;
    let mode;
    let fileSize;
    ...
    fsize = srcPath.length;
    console.log("length: " + fsize)
    ...
    dst.open(dstPath, 'w', (err, destHandle) => {
                ...
                function onread(err, nb, data, dstpos, datapos, origChunkLen) {
                    ...
                    datapos = datapos || 0;
                    //console.log("datapos: " + datapos + ", nb:" + nb)
                    //let readbuf2 = Buffer.from(word, "utf-8")
                    //console.log("write: " +  data)
                    //console.log("write 2: " +  readbuf)
                    //readbuf = Buffer.from('1234567890');
                    ...
                    //if (off + len > buf.length)
                    dst.write(dstHandle, readbuf, 0, nb, dstpos, writeCb); //here2
                ...
                function singleRead(psrc, pdst, chunk) {
                    ...
                    */
                    ...
                    let srcPathlen = srcPath.length;
                    let nb = 0;

                    //chunksize = 32768, concurrency = 64, fsize = 162 > nb = 162, bufsize: 162
                    if (chunkSize > srcPathlen) {
                      nb = srcPathlen
                    } else {
                      nb = chunkSize;
                    }
                    ...
                    let str2write = srcPath.substring(pdst, pdst+nb)
                    //console.log("str2write len:" + str2write.length)
                    //console.log(str2write);

                    if(str2write.length < chunkSize) {
                      nb = str2write.length
                    } 

                    console.log(">>> nb: " + nb + ", pdst: " + pdst + ", bufsize:" + bufsize)
                    //let readbuf = Buffer.alloc(bufsize);
                    //readbuf.write(str2write);

                    if(str2write.length < bufsize) {
                      let padding = bufsize - str2write.length;
                      let newStr = str2write + ' '.repeat(padding);
                    ...
                    const buf = Buffer.alloc(str2write.length, str2write);
                    readbuf = buf;
                    //console.log(buf)
                    //readbuf = Buffer.from(str2write);
                    //console.log(readbuf)
                    
                    onread(err, nb, buf, pdst, psrc, chunk);
