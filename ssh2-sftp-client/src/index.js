...
class SftpClient {
  ...
  _put(lPath, rPath, opts) {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log("VVVVVVVVVVVddddd A")
      wtr.once('error', (err) => {
        reject(this.fmtError(`${err.message} ${rPath}`, 'put', err.code));
      });
      wtr.once('close', () => {
        console.log(`Uploaded data stream to ${rPath}`);
        resolve(`Uploaded data stream to ${rPath}`);
      });
      if (lPath instanceof Buffer) {
        console.log("VVVVVVVVVVVddddd B")
        this.debugMsg('put source is a buffer');
        wtr.end(lPath);
        console.log("VVVVVVVVVVVddddd End")
      } else {
        console.log("VVVVVVVVVVV C")
        rdr =
          typeof lPath === 'string'
  ...
  async put(localSrc, remotePath, options) {
    ...
    try {
      ...
      let x = await this._put(localSrc, remotePath, options);
      console.log("EEENDD")
      return x
    } catch (e) {
